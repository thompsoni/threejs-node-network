		...
		fragmentShader: [
			...
			"float calcLightAttenuation( float lightDistance, float cutoffDistance, float decayExponent ) {",
 				"if ( decayExponent > 0.0 ) {",
 					"return pow( abs(saturate( - lightDistance / cutoffDistance + 1.0 )), decayExponent );",
 				"}",
 				"return 1.0;",
 			"}",
			...
			"void main() {",
				...
				"if( enableDiffuse1 && enableDiffuse2 ) {",
					...
					"vec4 colNoise = texture2D( tDisplacement, uvBase );",

					"colDiffuse1 = GammaToLinear( colDiffuse1, float( GAMMA_FACTOR ) );",
					"colDiffuse2 = GammaToLinear( colDiffuse2, float( GAMMA_FACTOR ) );",
					"colNoise = GammaToLinear( colNoise, float( GAMMA_FACTOR ) );",

					"diffuseColor *= mix ( colDiffuse1, colDiffuse2, colNoise );",
				...
				"#if NUM_DIR_LIGHTS > 0",
					...
					"for( int i = 0; i < NUM_DIR_LIGHTS; i++ ) {",
						...
						"float dirSpecularWeight = specularTex.r * max( pow( abs(dirDotNormalHalf), shininess ), 0.0 );",
				...
				"#if NUM_HEMI_LIGHTS > 0",
					...
					"for( int i = 0; i < NUM_HEMI_LIGHTS; i ++ ) {",
						...
						"hemiSpecularWeight += specularTex.r * max( pow( abs(hemiDotNormalHalfSky), shininess ), 0.0 );",
						...
						"hemiSpecularWeight += specularTex.r * max( pow( abs(hemiDotNormalHalfGround), shininess ), 0.0 );",
